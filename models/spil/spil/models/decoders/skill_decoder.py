...
class SkillDecoder(ActionDecoder):
    def __init__(
        ...
        if not self.sg_chk_path.is_absolute():
            self.sg_chk_path = Path(skill_generator.__file__).parent.parent / self.sg_chk_path
    ...
    def _base_skill_reg_loss(
        ...
        Returns:
            base_skill_reg_loss: the loss to regularize the skills based on the selected base skills
        """
